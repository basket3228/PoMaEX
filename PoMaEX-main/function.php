...
{
    $pdo = get_connection();
    $st = $pdo->prepare("select distinct LGparty.LGparty_id, t1.EXable as type1, t2.EXable as type2, t3.EXable as type3, t1.EXable as fa1, t2.EXable as fa2, t3.EXable as fa3, t1.EX_Flg as f1, t2.EX_Flg as f2, t3.EX_Flg as f3, t1.stars as ts1, t2.stars as ts2, t3. stars as ts3, t1.trainer_name as t1, t2.trainer_name as t2, t3.trainer_name as t3, s1.sync_name as s1, s2.sync_name as s2, s3.sync_name as s3, t1.level as l1, t2.level as l2, t3.level as l3, LGParty.enemy_id from LGParty
...
{
    $pdo = get_connection();
    $st = $pdo->prepare("select distinct SAparty.SAparty_id, trainer.trainer_name, trainer.level from SA
...
{
    $pdo = get_connection();
    $st = $pdo->prepare("select trainer_id, trainer_name, sync.sync_name, sync.type_id, role.role_name, EX_flg, potential, level, stars, URL from trainer
    left join sync on sync.sync_id = trainer.sync_id
...
{
    $pdo = get_connection();
    $st = $pdo->prepare("select trainer_id, trainer_name, sync.sync_name, sync.type_id, role.role_name, EX_flg, potential, level, stars from trainer
    left join sync on sync.sync_id = trainer.sync_id
...
{
    $pdo = get_connection();
    $st = $pdo->prepare("select trainer_id, trainer_name, sync.sync_name, role.role_name, EX_flg, potential, level, stars from trainer
    left join sync on sync.sync_id = trainer.sync_id
